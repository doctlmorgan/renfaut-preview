...
function buildShareText(results){
  ...
  return `${results.name || 'Participant'}'s R.I.S.E. Index™ Leadership Snapshot: ${results.configuration?.name || 'Leadership Profile'}. Principle scores — ${scores}.`;
}
...
function setupActions(results){
  ...
  $('shareReport').addEventListener('click', async event => {
    ...
    try{
      ...
      const shareData = {
        title: `${firstValue(results.name, results.participant?.name, results.demographics?.name, 'Participant')} — R.I.S.E. Index™ Leadership Snapshot`,
        text: 'R.I.S.E. Index™ Leadership Snapshot',
        files: [file]
      };
...
function renderEmptyState(){
  document.body.innerHTML = `<main class="snapshot-empty"><h1>No R.I.S.E. results were found</h1><p>Please complete the inventory before opening this page.</p><p><a href="rise-inventory.html">Return to the inventory</a></p></main>`;
}
